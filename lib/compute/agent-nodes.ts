  ...
  constructor(stack: Stack) {
    ...
    this.WINDOWS2019_X64_DOCKER_HOST = {
      ...
      initScript: 'echo %USERNAME% && dockerd --register-service && net start docker && echo started docker deamon && docker ps && '
          + 'echo initializing docker images now waiting for 5min && git clone https://github.com/distronode-project/distronode-build.git && '
          + 'bash.exe -c "docker run --rm -it  --name docker-windows-test -d `distronode-build/docker/ci/get-ci-images.sh '
          + '-p windows2019-servercore -u distronode -t build | head -1` bash.exe && sleep 5" && docker exec docker-windows-test whoami && '
          + 'docker ps && docker stop docker-windows-test && docker ps && rm -rf distronode-build',
