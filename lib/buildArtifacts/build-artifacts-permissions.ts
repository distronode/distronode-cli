...
export class BuildArtifactsPermissions {
  private static readonly BUNDLE_ROLE_NAME = 'distronode-bundle';

  constructor(stack: CiCdnStack, props: buildArtifactProps) {
    const distronodeBundleRole = new Role(stack, BuildArtifactsPermissions.BUNDLE_ROLE_NAME, {
      assumedBy: Role.fromRoleArn(stack, 'MainNodeRole', `${props.mainNodeArn}`),
      roleName: 'distronode-bundle',
    });

    distronodeBundleRole.assumeRolePolicy?.addStatements(new PolicyStatement({
  ...
  private static getDistronodeBundlePolicies(scope: Stack, s3BucketArn: string): Policy[] {
    ...
    const openSearchBundlePolicy = new Policy(scope, 'distronode-bundle-policy', {
      statements: [
        new PolicyStatement({
          actions: [
            's3:GetObject*',
